if active
{
    ...
    if ((oControl.kLeft > 0 && oControl.kLeftPushedSteps == 0) || (oControl.kRight > 0 && oControl.kRightPushedSteps == 0 && global.curropt < 13 && oControl.kDown == 0 && oControl.kUp == 0))
    {
        if (global.curropt == 0)
        {
            if (os_type != os_android)
        ...
        if (global.curropt == 12)
            global.opcorexsensitivity = (!global.opcorexsensitivity)
        if (os_type == os_android)
        {
            if (global.curropt != 0)
                sfx_play(sndMenuMove)
