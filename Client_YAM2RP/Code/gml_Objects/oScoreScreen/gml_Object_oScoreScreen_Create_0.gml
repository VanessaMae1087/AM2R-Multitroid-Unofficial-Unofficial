...
text1a = steps_to_time2(global.endingGametime);
text2 = get_text("ScoreScreen", "ItemCollection");
text2a = string(round((global.endingItemstaken / 88) * 100)) + "%";
...
statetime = 0;
state = 0;
itemstaken = global.endingItemstaken;
...
{
    if (global.endingGametime >= 864000)
    {
        ending = 278;
        unlock_gallery(0);
    }
    
    if (global.endingGametime >= 432000 && global.endingGametime < 864000)
    {
        ending = 279;
        unlock_gallery(1);
        unlock_gallery(0);
    ...
    if (global.endingGametime < 432000)
    {
        ending = 280;
        unlock_gallery(2);
        unlock_gallery(1);
